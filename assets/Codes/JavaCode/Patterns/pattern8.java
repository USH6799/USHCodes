...
//bachooo. Explanation:-
    ...
    whenever i and j are equal , The stars Were Printed
    like:-  i=1,j=1 [i = j] ---> "*"
    i=2,j=2 [i = j] ---> "*" 
    i=3,j=3 [i = j] ---> "*"
    i=4,j=4 [i = j] ---> "*"
    ...
    which means if i and j are not equal it checks the next condition and the next condition says if the sum of i and j is equal to 6 then print the stars
    ...
    in this New table, U can see the stars which were print only by the 2nd condition  (i + j = 6)
    
    like,
    1st star -> i=1:j=5 => [i + j = 6] --> true
    2nd star -> i=2:j=4 => [i + j = 6] --> true
    3rd star -> i=3:j=3 => [i + j = 6] --> true
    4th star -> i=4:j=2 => [i + j = 6] --> true
    5th star -> i=5:j=1 => [i + j = 6] --> true
